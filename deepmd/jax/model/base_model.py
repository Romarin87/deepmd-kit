...
def forward_common_atomic(
    ...
    def get_atomic_ret() -> dict[str, jnp.ndarray]:
        # Materialize atomic outputs lazily. For force paths, jacrev(has_aux=True)
        # can return the primal atomic outputs without an extra forward pass.
        nonlocal atomic_ret
        ...
        if vdef.reducible and vdef.r_differentiable:
            ...
            def eval_output(
                ...
                *,
                ...
                # Strip the singleton frame dimension so the vmapped aux tree
                # matches the batched shape from a direct atomic forward call.
                ...
                if do_atomic_virial:
                    ...
                    def eval_ce(
                        ...
                        *,
                        ...
                        # virial correction, so it needs its own transformed
                        # forward through atomic_ret[_kk].
